...
#else
template <typename Grammar, const auto & input, typename ActionSelector = empty_actions, bool IgnoreUnknownActions = false> struct uigen {
#endif
	...
	template <size_t Pos, typename Stack, typename Subject> static constexpr auto decide(Stack previous_stack, Subject previous_subject) noexcept {
		...
		if constexpr (std::is_base_of_v<ctll::action, decltype(top_symbol)>) {
			...
			// all other cases are ordinary for LL(1) uigen
...
// requires ctll::uigen<ctre::pcre, _fixed_string_reference<CharT, charpack...>, ctre::pcre_actions>::template correct_with<pcre_context<>>
